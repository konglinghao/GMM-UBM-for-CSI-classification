function new_mix=mapAdapt(mix,data,r)
...
%map data structure to work with my code...
mix.nin = size(data, 2);
mix.ncentres = size(mix.w, 2);
mix.priors = mix.w;
mix.covars = mix.sigma;
mix.centres = mix.mu;
mix.covar_type = 'full';

input_dim=mix.nin;                %No of input i.e. Feature Dimension = 30
[post, act] = gmmpost(mix, data); %Computes class posterior probabilities
prob = act*(mix.priors)';
new_pr = sum(post, 1);
alpha=new_pr./(new_pr+r);
new_c = post' * data;
mix.centres = repmat(alpha',1,input_dim).*(new_c ./ (new_pr' * ones(1, input_dim)))...
    +(1-repmat(alpha',1,input_dim)).*mix.centres;

%map again
new_mix.w = mix.priors;
new_mix.sigma = mix.covars;
new_mix.mu = mix.centres;